...
//                    }
                    ...
                    try {
                        ...
                        logViewln("发送" + StringTool.byteHexToSting(cmd));
                        byte[] rsp = cpuCard.transceive(cmd);
                        ...
                        logViewln("测试次数：" + cnt++);
                        logViewln("测试时间：" + ((System.currentTimeMillis() - time_start) / 1000) + "秒" );
                        cpuCard.close();
                    } catch (CardNoResponseException e) {
                        e.printStackTrace();
                    }
